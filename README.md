...
Now that the release is completed, the private git repository should be "synced" with the public reposititory by running [this script](Scripts/sync_public_repo.sh). It pushes new *non-RC* tags to the public repository (or run `make sync_repo` from the command line).
