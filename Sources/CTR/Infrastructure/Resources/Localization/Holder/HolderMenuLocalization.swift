/*
* Copyright (c) 2021 De Staat der Nederlanden, Ministerie van Volksgezondheid, Welzijn en Sport.
*  Licensed under the EUROPEAN UNION PUBLIC LICENCE v. 1.2
*
*  SPDX-License-Identifier: EUPL-1.2
*/
  
import Foundation

// MARK: - Holder

extension String {

	static var holderMenuDashboard: String {

		return Localization.string(for: "holder.menu.dashboard")
	}

	static var holderMenuSettings: String {

		return Localization.string(for: "holder.menu.settings")
	}

	static var holderMenuFaq: String {

		return Localization.string(for: "holder.menu.faq")
	}

	static var holderMenuAbout: String {

		return Localization.string(for: "holder.menu.about")
	}

	static var holderMenuFeedback: String {

		return Localization.string(for: "holder.menu.feedback")
	}

	static var holderMenuPrivacy: String {

		return Localization.string(for: "holder.menu.privacy")
	}
}
